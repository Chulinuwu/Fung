<script lang="ts">
  ...
  async function fetchLikedSongs(accessToken: string) {
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            tracks = data.items.map((item: any) => item.track);
            console.log('🎵 Liked Songs loaded:', tracks);
        } else {
            console.error('❌ Error fetching liked songs:', await response.json());
        }
    } catch (error) {
        console.error('❌ Network error:', error);
    }
  ...
  onMount(async () => {
      ...
      if (!accessToken || !checkTokenExpiry(accessToken)) {
          ...
          if (!window.location.hash.includes('access_token')) {
              ...
              const scope =
              'streaming user-read-private user-read-email user-modify-playback-state user-read-playback-state playlist-read-private user-top-read user-read-recently-played user-library-read';
                ...
                <li>
                  <button
                      ...
                      on:click={async () => {
                          ...
                          tracks = likedSongs.map(item => item.track);
                      }}
                  >
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
